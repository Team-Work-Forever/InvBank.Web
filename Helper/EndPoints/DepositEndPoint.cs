...
public class DepositEndPoint : BaseEndPoint
{
    ...
    public async Task<ErrorOr<IEnumerable<DepositResponse>>> GetAllDeposit(string accountIban)
    {
        ...
        (
            () => _apiHelper.DoGet($"/deposits/all?iban={accountIban}", "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6InplQGdtYWlsLmNvbSIsInJvbGUiOiIwIiwidXNlcklkIjoiODAwNWU1MGQtNDE2OS00ZjQwLTlmYzMtZjk2YTY5YjgwNmI5IiwiZXhwIjoxNjg3MDkwMTM4LCJpc3MiOiJJbnZlc3RtZW50QmFuayIsImF1ZCI6IkludmVzdG1lbnRCYW5rIn0.4aHK7sC1RjGfeXWUmLOy1Bi22GXJqDqZ30nJD6uGcgI")
        );
    }

    public async Task<ErrorOr<IEnumerable<DepositResponse>>> CreateDeposit(CreateDepositRequest request)
    {
        return await MakeRequest<IEnumerable<DepositResponse>>
        (
